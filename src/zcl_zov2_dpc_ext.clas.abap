...
CLASS ZCL_ZOV2_DPC_EXT IMPLEMENTATION.
  ...
  METHOD /iwbep/if_mgw_appl_srv_runtime~create_deep_entity.
    ...
    DATA : ls_deep_entity  TYPE zcl_zov2_mpc_ext=>ty_ordem_item.
    DATA : ls_deep_item    TYPE zcl_zov2_mpc_ext=>ts_ovitem.

    DATA : ls_cab          TYPE zov2cab.
    DATA : lt_item         TYPE STANDARD TABLE OF zov2item.
...
*      ls_cab-criacao_usuario = ls_deep_entity-criadopor.
    ...
    " persistência itens
    DELETE FROM zov2item WHERE ordemid = ls_cab-ordemid.
    IF lines( lt_item ) > 0.
      INSERT zov2item FROM TABLE lt_item.
      IF sy-subrc <> 0.
        ROLLBACK WORK.
